 ...
 */
public final class JavaBeanRuleOutput implements RuleOutput, TypeInfo {
  private String name;
  private Class<?> type;
  ...
  public JavaBeanRuleOutput(String name, Class<?> javaBeanType) {
    ...
    for (PropertyDescriptor property : propertys) {
      ...
      if (fieldType == null || TypeInfo.isSimpleType(fieldType) || fieldType.isEnum()) {
        continue;
      }
      atLeastOnce = true;
      break;
    ...
    if (!atLeastOnce) {
      throw new IllegalArgumentException(String.format("type:%s，不是有效的JavaBean，至少包含一个类型为：Java八种基本类型以及String的Getter/Setter", type.getName()));
    }
